...
class ParsingZigbang:
    ...
    def parse_data(self):
        ...
        # ENUM 변환
        df["direction"] = df["direction"].apply(map_zigbang_direction)
        df["transaction_type"] = df["transaction_type"].apply(map_zigbang_transaction_type)
        df["property_type"] = df["property_type"].apply(map_zigbang_property_type)

        # property_id 형식 변환
        df['crawling_properties_id'] = df['crawling_properties_id'].apply(lambda x: f"zigbang_{x}")
